...
use super::{
    ...
    EffectiveReviewDecision, EffectiveReviewSet, ImageDimensions,
    asset_program_check::{check_image, detect_actual_type},
};

const JPEG_QUALITY: u8 = 90;

/// Minimal storage capability needed by Asset Sanitization.
pub trait AssetContentStore {
    fn read(&self, identity: Sha256) -> Result<Vec<u8>, ContentStoreError>;
    fn store(&self, content: &[u8]) -> Result<Sha256, ContentStoreError>;
}

impl AssetContentStore for LocalContentStore {
    fn read(&self, identity: Sha256) -> Result<Vec<u8>, ContentStoreError> {
...
pub struct AssetSanitizer<S = LocalContentStore> {
    content_store: S,
}

impl AssetSanitizer<LocalContentStore> {
    pub fn new(content_store: LocalContentStore) -> Self {
        Self { content_store }
    }
}

impl<S: AssetContentStore> AssetSanitizer<S> {
    ...
    fn sanitize_one(
        ...
        if !needs_reencode {
            return Ok(SanitizedAsset {
                path: path.clone(),
                review_run_id,
                source_sha256: file.sha256(),
                published_sha256: file.sha256(),
                published_size: file.size(),
                transformations: vec![SanitizationTransformation::Identity],
            });
        }
        ...
        Ok(SanitizedAsset {
            ...
            source_sha256: file.sha256(),
            published_sha256,
            published_size: usize_to_u64(published.len()),
            transformations: vec![
...
mod tests {
    ...
    impl AssetContentStore for TestStore {
        fn read(&self, identity: Sha256) -> Result<Vec<u8>, ContentStoreError> {
    ...
    fn fixture(
        ...
        (
            ...
            .unwrap(),
            EffectiveReviewSet::from_assets_for_test(id, reviews),
            AssetCheckResult::from_assets_for_test(id, checks),
        )
    }
