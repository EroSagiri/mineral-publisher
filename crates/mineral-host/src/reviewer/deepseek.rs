...
use crate::{
    domain::Sha256,
    policy::{ReviewCandidate, Reviewer, ReviewerError, ReviewerErrorKind, ReviewerReport},
    storage::LocalContentStore,
};

pub const MARKDOWN_REVIEWER_PROMPT_VERSION: &str = "mineral-markdown-publication-safety-v2";
...
{"decision":"approve","reason_codes":["ordinary_personal_content","public_technical_content"],"summary":"内容属于普通个人生活与公开技术学习记录。"}
...
{"decision":"needs_human_review","reason_codes":["unpublished_product_or_technical_plan","uncertain_disclosure_authorization"],"summary":"涉及未公开技术计划，其公开授权无法确认。"}
...
{"decision":"reject","reason_codes":["credential_secret"],"summary":"文档包含疑似真实生产环境访问凭据。"}
...
{"decision":"approve","reason_codes":["ordinary_work_experience"],"summary":"内容属于普通工作经历，没有披露具体敏感材料。"}
...
{"decision":"needs_human_review","reason_codes":["customer_or_partner_information","unpublished_product_or_technical_plan","uncertain_disclosure_authorization"],"summary":"涉及客户信息和未公开技术计划，其公开授权无法确认。"}
...
{"decision":"reject","reason_codes":["credential_secret","third_party_private_information"],"summary":"文档包含第三方系统的疑似真实私密访问凭据。"}
...
必须返回符合 ReviewerReport contract 的 valid json。

summary 只提供一到两句简短判定依据，不输出详细推理过程，不引用敏感原值，
不复制秘密、凭据、完整联系方式、地址、身份证明或长段原文。
...
fn output_contract_instruction() -> String {
    ...
    let schema =
        serde_json::to_string(&schema).expect("ReviewerReport JSON Schema is always serializable");
    format!(
        "PROGRAM OUTPUT CONTRACT\n\
         ...
         JSON Schema: {schema}\n\
         The summary must be one or two short sentences and must not reproduce secrets, credentials, personal identifiers, complete contact details, addresses, or long source passages.\n\
         Return only a brief classification explanation, never chain-of-thought or detailed reasoning."
    )
}
...
fn parse_response(
    ...
    let report: ReviewerReport = serde_json::from_str(content).map_err(|_| {
        reviewer_error(
            ReviewerErrorKind::MalformedResponse,
            "DeepSeek review classification did not match the ReviewerReport contract",
        )
    })?;
    Ok(report)
}
...
mod tests {
    ...
    use crate::{
        ...
        policy::{
            HumanReviewReason, PolicyIdentity, PublicPolicyDecision, ReviewDecision, ReviewRunId,
            ReviewRunStore, ReviewerErrorKind,
        },
        source::LocalSource,
    ...
    fn completion(content: &str) -> FakeResponse {
        let content = serde_json::from_str::<Value>(content)
            ...
            .and_then(|mut value| {
                ...
                {
                    let reason = match object.get("decision")?.as_str()? {
                        "approve" => "public_technical_content",
                        _ => "other_privacy_risk",
                    };
                    object.insert("reason_codes".to_owned(), json!([reason]));
    ...
    fn run(
        ...
        let result = PublicPolicyRun::execute_at(
            snapshot,
    ...
    fn structured_classification_requires_safe_findings() {
        let valid: ReviewerReport = serde_json::from_str(
            r#"{"decision":"reject","reason_codes":["credential_secret","private_contact_information"],"summary":"文档包含敏感信息。"}"#,
        )
        .unwrap();
        assert_eq!(valid.decision(), ReviewDecision::Reject);

        for invalid in [
            r#"{"decision":"approve","summary":"missing codes"}"#,
            r#"{"decision":"reject","reason_codes":[],"summary":"empty codes"}"#,
            ...
            r#"{"decision":"approve","reason_codes":["public_technical_content","public_technical_content"],"summary":"duplicate codes"}"#,
            r#"{"decision":"approve","reason_codes":["ordinary_personal_content"],"summary":" "}"#,
        ] {
            assert!(serde_json::from_str::<ReviewerReport>(invalid).is_err());
        }
    }
    ...
    fn prompt_has_stable_version_and_content_hash() {
        ...
        assert_eq!(
            ...
            "mineral-markdown-publication-safety-v2"
        );
        assert_eq!(
