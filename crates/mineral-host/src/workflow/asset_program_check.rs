...
use crate::{
    domain::{ContentPath, Snapshot, SnapshotFile, SnapshotId},
    ports::{BlobStore, ContentStoreError},
};

use super::{
    ActualAssetType, AssetCheckFinding, AssetCheckResult, AssetInspector, CandidateAssetSet,
    CheckedAsset, ImageDimensions,
};

/// Checks only the assets already selected by `CandidateAssetSet`.
#[derive(Clone, Debug)]
pub struct AssetProgramCheck<B: BlobStore> {
    content_store: B,
}

impl<B: BlobStore> AssetProgramCheck<B> {
    ...
    pub fn run(
        ...
        for candidate in candidates.entries() {
            let Some(file) = snapshot
                ...
                assets.push(CheckedAsset::new(
                    ...
                    None,
                    None,
                    vec![AssetCheckFinding::SnapshotFileMissing],
                ));
                continue;
            };
        ...
        Ok(AssetCheckResult::from_assets(snapshot.id(), assets))
    }
}
...
/// The native implementation of the engine's [`AssetInspector`] port.
///
/// It decodes real bytes through `image`/`exif`, which is exactly the capability
/// the engine must not assume: another runtime implements the same port without
/// ever holding the bytes in its own memory.
impl<B: BlobStore> AssetInspector for AssetProgramCheck<B> {
    type Error = AssetProgramCheckError;
